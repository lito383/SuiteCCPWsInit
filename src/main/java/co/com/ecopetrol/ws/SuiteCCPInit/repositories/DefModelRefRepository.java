package co.com.ecopetrol.ws.SuiteCCPInit.repositories;

import co.com.ecopetrol.ws.SuiteCCPInit.entities.DefModelRef;
import java.util.List;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;

/**
 *
 * @author STC
 */
public interface DefModelRefRepository extends JpaRepository<DefModelRef, Long> {

    @Query("SELECT obj FROM DefModelRef obj WHERE obj.estBorrado = 0 ORDER BY obj.id ASC")
    public List<DefModelRef> getLstAllDefModelRef();

}
